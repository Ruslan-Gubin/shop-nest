...
describe('OrdersService — setShortageStocks', () => {
  ...
  describe('A. Валидация заказа', () => {
    it('заказ не найден → ошибка', async () => {
      ...
      await expect(service.setShortageStocks(999, [...items])).rejects.toBe(
        'Заказ 999 не найден',
      );
    ...
    it.each(['new', 'processing'])('статус «%s» → ок', async (status) => {
      ...
      await expect(
        service.setShortageStocks(1, [...items]),
      ).resolves.toBeUndefined();

      expect(mockOrderProductService.setShortageStocks).toHaveBeenCalledTimes(
        1,
      );
    });
  });
