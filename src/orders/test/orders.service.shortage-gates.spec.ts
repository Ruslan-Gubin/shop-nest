...
describe('OrdersService — shortage-гейты (ship, changeStatus)', () => {
  ...
  describe('ship', () => {
    it('блокируется, пока есть дефицит', async () => {
      ...
      await expect(
        ...
        'Заказ 1 ожидает решения клиента по изменению количества товаров в заказе',
      );
    ...
    it('без order_id в transfers → «Заказ undefined не найден»', async () => {
      await expect(
        service.ship({ transfers: [], reservations: [] } as any),
      ).rejects.toBe('Заказ undefined не найден');
  ...
  describe('changeStatus', () => {
    ...
    it('new → processing без дефицита — работает', async () => {
      ...
      expect(mockOrdersRepository.update).toHaveBeenCalledWith(1, {
        status: 'processing',
      });
    });
  });
