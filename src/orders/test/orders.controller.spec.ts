...
describe("OrdersController — shortage endpoints", () => {
  ...
  describe("PATCH shortage/:id", () => {
    ...
    it("вызывает сервис с Number(id) и dto.items, возвращает success", async () => {
      ...
      expect(mockOrdersService.setShortageStocks).toHaveBeenCalledWith(42, dto.items);
    ...
    it("возвращает error с текстом ошибки сервиса", async () => {
      ...
      expect(result.data).toBeNull();
      expect(result.message).toBe("Невозможно установить дефицит для заказа в статусе completed");
    });

    it("извлекает message из Error", async () => {
      mockOrdersService.setShortageStocks.mockRejectedValue(new Error("DB failed"));
    ...
    it("защищён ролями admin и moderator", () => {
      const roles = Reflect.getMetadata(ROLES_KEY, OrdersController.prototype.setShortageStocks);
  ...
  describe("POST accept-shortage/:id", () => {
    it("вызывает сервис с user.sub и user.role, возвращает success", async () => {
      ...
      expect(mockOrdersService.acceptShortage).toHaveBeenCalledWith(7, mockUser.sub, mockUser.role);
      expect(result.status).toBe("success");
      expect(result.data).toBeNull();
    ...
    it("доступен без ограничения по ролям (владелец, admin, moderator)", () => {
      const roles = Reflect.getMetadata(ROLES_KEY, OrdersController.prototype.acceptShortage);
