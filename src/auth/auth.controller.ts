...
export class AuthController {
  ...
  async signIn(@Body() data: SignInDto): Promise<ResponseData<Tokens | null>> {
    try {
      ...
      return responseData(tokens, "success", [], "Пользователь успешно вошел в систему");
    } catch (error) {
  ...
  async logout(@CurrentUser() user: CurrentStrategyUser): Promise<ResponseData<null>> {
    try {
      ...
      return responseData(null, "success", [], "Пользователь успешно вышел");
    } catch (error) {
      return responseData(null, "error", [], error);
    }
  }
  ...
  ): Promise<ResponseData<Tokens | null>> {
    try {
  ...
  @Public()
  @Post("/verify-otp")
  async verifyOtp(@Body() dto: VerifyOtpDto): Promise<ResponseData<Tokens | null>> {
    try {
      const tokens = await this.authService.verifyOtp(dto);

      return responseData(tokens, "success", [], "Успешный вход по коду");
    } catch (error) {
      return responseData(null, "error", [], error);
    }
  }
