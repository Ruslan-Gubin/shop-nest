...
export class WarehouseController {
  ...
  > {
    try {
      ...
      const totalCount = await this.warehouseService.getTotalCount(
        user.sub,
        name,
      );
  ...
  async findPublic(): Promise<ResponseData<Warehouse[] | null>> {
    try {
      ...
      return responseData(
        ...
        'success',
        [],
        'Список активных складов получен',
      );
    } catch (error) {
      return responseData(null, 'error', [], error);
    }
    ...
    try {
      ...
      if (warehouse && warehouse.default_warehouse) {
        const updateWarehouse =
          await this.warehouseService.findNotDefaultWarehouse(
