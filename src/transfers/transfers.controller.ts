...
export class TransfersController {
  ...
  > {
    try {
      const [transfers, totalCount] = await this.transfersService.findAll(
        page,
        limit,
        status,
      );
  ...
  async findDeliveryByOrderId(
    ...
    try {
      const transfers = await this.transfersService.findDeliveryByOrderId(
        Number(id),
      );
      ...
      return responseData(
        ...
        'success',
        [],
        'Перемещения доставки заказа получены',
      );
    } catch (error) {
      return responseData(null, 'error', [], error);
    }
  ...
  > {
    try {
      ...
      const [transfers, totalCount] =
        await this.transfersService.findAllInTransit(pageNum, limitNum);
