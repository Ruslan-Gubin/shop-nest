...
describe('SpecificationsController', () => {
  ...
  beforeEach(async () => {
    ...
    ) as jest.Mocked<SpecificationsService>;
  ...
  describe('findAll', () => {
    ...
    it('должен фильтровать по name', async () => {
      ...
      expect(mockSpecificationsService.findAll).toHaveBeenCalledWith(
        '1',
        '10',
        'Мате',
      );
      expect(mockSpecificationsService.getTotalCount).toHaveBeenCalledWith(
        'Мате',
      );
    });

    it('должен вернуть ошибку при получении списка', async () => {
      mockSpecificationsService.findAll.mockRejectedValue(
        new Error('DB error'),
      );
  ...
  describe('findOne', () => {
    ...
    it('должен вернуть ошибку при поиске', async () => {
      mockSpecificationsService.findOne.mockRejectedValue(
        new Error('DB error'),
      );
  ...
  describe('update', () => {
    it('должен обновить характеристику', async () => {
      ...
      mockSpecificationsService.update.mockResolvedValue({
        affected: 1,
      } as any);
      ...
      expect(mockSpecificationsService.update).toHaveBeenCalledWith(
        1,
        updateDto,
      );
    });
  ...
  describe('remove', () => {
    it('должен удалить характеристику', async () => {
      mockSpecificationsService.remove.mockResolvedValue({
        affected: 1,
      } as any);
